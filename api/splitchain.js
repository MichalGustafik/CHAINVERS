// /api/chainvers.js
// Jeden router pre Circle utility + SplitChain (Hobby limit fix)
...
export default async function handler(req, res) {
  try {
    ...
    // --- diagnostika prostredia (pomôže pri chybách) ---
    const diag = {
      action,
      base: BASE,
      has_key: Boolean(key),
    ...
    async function readJsonBody(req) {
      if (req.body && typeof req.body === "object") return req.body; // Next už mohol parse-núť
      const chunks = [];
    ...
    // === NOVÉ: SPLITCHAIN (prenesené zo /api/splitchain.js) ===
    if (action === "splitchain") {
      ...
      // ak prišiel priamo Stripe event
      ...
      // 2) ETH (voliteľné – voláš svoj interný /api/coinbase_send ak máš)
      ...
      // 3) Profit payout (voliteľné – Stripe Payouts)
