    ...
    try {
        ...
        if (action === "withdraw") {
            ...
            const gas = await contract.methods
                .backendWithdraw(user, amountWei.toString())
                .estimateGas({ from: owner.address });

            const gasPrice = BigInt(await web3.eth.getGasPrice());
            const gasCost = BigInt(gas) * gasPrice;

            console.log("[GAS]", gas, gasPrice.toString(), gasCost.toString());
            ...
            if (finalWei <= 0n) {
                return res.json({
                    ok:false,
                    error:"Zadaná suma je príliš nízka (nepokryje gas)"
                });
            }

            console.log("[FINAL AMOUNT]", finalWei.toString());

            const tx = await contract.methods
                .backendWithdraw(user, finalWei.toString())
                .send({ from: owner.address, gas });

            console.log("[TX OK]", tx.transactionHash);
            return res.json({ ok:true, tx: tx.transactionHash });
        }
