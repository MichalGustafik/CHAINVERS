...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (
      action === "chaingetcashdraw" ||
      action === "chaingetcash_draw"
    ) {
      return chainGetCashDrawAction(req, res);
    }
...
function chainGetCashDrawLogger() {
  const rows = [];
...
async function chainGetCashDrawAction(req, res) {
  const log = chainGetCashDrawLogger();
  ...
  try {
    ...
    const withdrawTo = String(
      body.withdraw_to || user
    ).toLowerCase().trim();
