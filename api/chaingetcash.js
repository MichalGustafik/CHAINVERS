...
async function getEurEthRate(){
  try{
    ...
    await log("⚠️ CoinGecko fail → 2500");return 2500;
  }
}
...
async function getBalanceEth(addr){
  try{
    ...
    await log(`⚠️ getBalance fail: ${e.message}`);return 0;
  }
}
...
/* === MINT COPY TRANSACTION === */
const MIN_FALLBACK_ETH = 0.00005; // minimálny poplatok ak je objednávka 0 €
...
async function mintCopyTx({ token_id, eurAmount, rate, gasPrice, mintFeeEth }) {
  ...
  if (balance.lt(gasCost.add(valueWei))) {
    ...
    if (fallbackWei.lt(balance)) {
      ...
      await log(`⚠️ Low balance → fallback value ${web3.utils.fromWei(valueWei)} ETH (token ${token_id})`);
    } else {
...
export default async function handler(req,res){
  try{
    ...
    await log(`💳 FROM_ADDRESS confirmed: ${FROM}`);
    await log(`🔐 PRIVATE_KEY loaded: ✓`);
    ...
    if(Number(balEth)<0.001){
      await log(`⚠️ Low ETH balance (${balEth}) – Dobite Coinbase pred mintom`);
    ...
    for(const o of orders){
      ...
      try{
        ...
        if(txHash){funded++;await markOrderPaid(o.paymentIntentId||String(token_id),txHash,o.user_address);}
