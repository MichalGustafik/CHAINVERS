...
/* === ABI (createCopy/mintCopy + mintFee) === */
const ABI = [
...
/* === SMART MINTER (pre 0€ len mintFee/fallback; kontrola gas+value) === */
...
async function mintCopyTx({ token_id, eurAmount, rate, gasPrice, mintFeeEth }) {
  ...
  let gasLimit;
  try {
    gasLimit = await contract.methods.mintCopy(token_id)
...
export default async function handler(req, res){
  try{
    ...
    // "fire and forget" (ignorujeme odpoveď)
    fetch(`${INF_FREE_URL}/accptpay.php?action=balance&val=${balEth}`).catch(()=>{});

    // ak nízky zostatok → len informovať v logu, nespúšťať mint
    if (Number(balEth) < 0.001) {
      await log(`⚠️ Low ETH balance (${balEth} ETH) → Dobite cez Coinbase pred mintom`);
